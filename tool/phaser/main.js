...
class StartMenu extends Phaser.Scene {
    ...
    create() {
        this.add.text(400, 150, 'Typing Game', {
            fontSize: '48px',
            color: '#ffffff'
        }).setOrigin(0.5);
...
class TypingGame extends Phaser.Scene {
    ...
    create() {
        this.words = ['apple', 'tree', 'cloud', 'sky', 'kiwi'];
        this.score = 0;
        this.timeLeft = 30;

        this.currentWord = Phaser.Utils.Array.GetRandom(this.words);

        this.wordText = this.add.text(400, 150, this.currentWord, {
            fontSize: '48px',
            color: '#3258a8'
        }).setOrigin(0.5);
        ...
        element.addEventListener('keyup', (event) => {
            if (event.key === 'Enter') {
                this.checkAnswer(element.value);
                element.value = '';
            }
        });
...
const config = {
    ...
    scene: [StartMenu, TypingGame, GameOver]
};
